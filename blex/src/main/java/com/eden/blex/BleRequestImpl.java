...
import com.eden.blex.BuildConfig;
import com.eden.blex.callback.wrapper.ConnectWrapperCallback;
...
import com.eden.blex.utils.ByteUtils;
import com.eden.blex.ota.OtaListener;
...
public final class BleRequestImpl<T extends BleDevice> {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    final BluetoothGatt bluetoothGatt = getBluetoothGatt(device.getAddress());
    ...
    public boolean isDeviceBusy(T device) {
        boolean state = false;
        try {
            final BluetoothGatt gatt = getBluetoothGatt(device.getBleAddress());
            if (gatt != null) {
    ...
    public boolean writeCharacteristicByUuid(String address, byte[] value, UUID serviceUUID, UUID characteristicUUID) {
        final BluetoothGatt gatt = getBluetoothGatt(address);
        if (gatt != null) {
            BluetoothGattCharacteristic characteristic = gattCharacteristic(gatt, serviceUUID, characteristicUUID);
    ...
    public boolean readCharacteristicByUuid(String address, UUID serviceUUID, UUID characteristicUUID) {
        final BluetoothGatt gatt = getBluetoothGatt(address);
        BluetoothGattCharacteristic gattCharacteristic = gattCharacteristic(gatt, serviceUUID, characteristicUUID);
    ...
    public boolean readDescriptor(String address, UUID serviceUUID, UUID characteristicUUID, UUID descriptorUUID) {
        final BluetoothGatt gatt = getBluetoothGatt(address);
        if (gatt != null) {
            final BluetoothGattCharacteristic gattCharacteristic = gattCharacteristic(gatt, serviceUUID, characteristicUUID);
            if (gattCharacteristic != null) {
    ...
    public void setCharacteristicNotification(String address, boolean enabled) {
        final BluetoothGatt gatt = getBluetoothGatt(address);
        if (gatt != null && !notifyCharacteristics.isEmpty()) {
            for (BluetoothGattCharacteristic characteristic : notifyCharacteristics) {
                setCharacteristicNotificationInternal(gatt, characteristic, enabled);
            }
        }
    }
    ...
    public List<BluetoothGattService> getSupportedGattServices(String address) {
        final BluetoothGatt bluetoothGatt = getBluetoothGatt(address);
        if (bluetoothGatt == null) {
            return null;
        }
    ...
    public boolean writeOtaData(String address, byte[] value) {
        try {
            ...
            if (otaWriteCharacteristic == null) {
                otaUpdating = true;
                final BluetoothGattService bluetoothGattService = gatt.getService(options.uuid_ota_service);
                if (bluetoothGattService == null) {
                    ...
                    if (mOtaNotifyCharacteristic != null) {
                        gatt.setCharacteristicNotification(mOtaNotifyCharacteristic, true);
                    }
            ...
            if (otaWriteCharacteristic != null && options.uuid_ota_write_cha.equals(otaWriteCharacteristic.getUuid())) {
                ...
                boolean result = writeCharacteristic(gatt, otaWriteCharacteristic);
