    ...
    private final ScanCallback scannerCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            if (scanWrapperCallback != null) {
                scanWrapperCallback.onLeScan(device, result.getRssi(), recordBytes);
            }

            if (Ble.options().isParseScanData) {
                ScanRecord parseRecord = ScanRecord.parseFromBytes(recordBytes);
                if (parseRecord != null && scanWrapperCallback != null) {
                    scanWrapperCallback.onParsedData(device, parseRecord, result.getRssi());
                }
            }
        }
